...
void nRF905::begin(SPIClass *spi, uint32_t spiClock, int csn, int trx, int tx,
...
#if !defined(ESP32) && !defined(ESP8266)
  // spi->usingInterrupts() is not supported on ESP platforms. More info:
  // https://github.com/zkemble/nRF905-arduino/issues/1
...
bool nRF905::TX(nRF905_nextmode_t nextMode, bool collisionAvoid) {
  ...
  if (currentMode == NRF905_MODE_POWERDOWN) {
    ...
    if (nextMode != NRF905_NEXTMODE_TX)
      delay(3); // Delay is needed to the radio has time to power-up and see the
                // standby/TX pins pulse
  ...
  if (nextMode == NRF905_NEXTMODE_RX) {
    ...
    // mode after
    //	a transmission is complete by clearing TX_EN while transmitting, but if
